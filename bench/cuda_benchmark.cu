...
CudaBenchmark::Results CudaBenchmark::SortKeyValue(const std::vector<uint32_t> &keys,
  ...
  // CPU to GPU
  cudaMemcpy(keys_ptr_, keys.data(), n * sizeof(uint32_t), cudaMemcpyDefault);
  cudaMemcpy(values_ptr_, values.data(), n * sizeof(uint32_t), cudaMemcpyDefault);

  // allocate temp storage
  size_t temp_storage_bytes = 0;
  cub::DeviceRadixSort::SortPairs(
      nullptr, temp_storage_bytes, static_cast<const uint32_t *>(keys_ptr_),
      static_cast<uint32_t *>(out_keys_ptr_), static_cast<const uint32_t *>(values_ptr_),
  ...
  // GPU to CPU
  cudaMemcpy(result.keys.data(), out_keys_ptr_, n * sizeof(uint32_t), cudaMemcpyDefault);
  cudaMemcpy(result.values.data(), out_values_ptr_, n * sizeof(uint32_t), cudaMemcpyDefault);
